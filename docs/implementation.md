...
result       0x3010 <番号>        → 0xB010     測定結果。保持件数分だけ繰り返す
```

### 複数件の取得

`0x3010` の payload は**レコード番号**（1 起点、1 が最新）。`0x3000` の応答 `0xB000` が
保持件数 N を返すので、`N → 1` の順に取れば全件そろう。Health Planet と同じ順序で、
JSON には `recordIndex` として番号が入る。

```bash
bc768-probe sync --json --out ~/bc768.jsonl
```

- 件数が 0 なら `0x3010` を送らずに `0x0001` へ進む（送っても残留値が返るだけのため）。
- 想定外の件数が返ったときの歯止めとして 32 件で頭打ちにする。

**セッションを終えると BC-768 側の保持データは消える。** 取りこぼした分は戻らないので、
`sync` は常に全件取りに行く（件数を絞るオプションは用意していない）。

本体だけで測ると溜まる。`measure` で測るとその場で引き取るので溜まらない。
