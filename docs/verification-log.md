...
この試行の前後で、**BC-768 本体に `Er C7` と表示され、その後電源が切れた**。

- 本体側でも異常として扱われている。`0x8003` の payload `0701` と対応している可能性がある。
- 送ったのはすべて HCI ログで観測済みの payload であり、推測データは含まない。
  それでも本体がエラーを出したことから、**長押しで入るモードは Android のキャプチャを取った状態とは
  別のモードである**可能性が高い。

`Er C7` の意味が分かるまで handshake の実機試行は止める。取扱説明書のエラー表示一覧で確認する。

### 現時点で残っている仮説

1. **モード違い**（最有力）。長押しは登録／設定モードで、Android のキャプチャは
   測定直後の自動送信モードのもの。後者では別の初期シーケンスが要る、または同じ手順が通る。
2. **クライアント識別子の扱い**。同じ識別子文字列を送っていても、BC-768 が BLE アドレスや
   ボンディング情報とセットで端末を判別している場合、macOS は未知の端末として扱われる。
3. 長押しモードには専用の登録コマンドがあり、`0x0003` から始めるのが誤り。

いずれも Health Planet の登録フロー（長押しを案内される場面）の HCI キャプチャがあれば直接確かめられる。
