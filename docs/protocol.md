...
| `6F21` | 2 | 体水分率 | 0.1 % | 推定 |
| `6F22` | 2 | インピーダンス? | — | 未解析 |
| `614B` | 2 | インピーダンス?（符号付き） | — | 未解析 |
| `614C` | 2 | インピーダンス?（符号付き、観測値は負） | — | 未解析 |

### 「確定」の根拠

体重・身長・体脂肪率・筋肉量・推定骨量・BMI は、次の 2 つが**独立に成立する**ことで裏づけた。
3 サンプル（Android 2 件 + macOS 1 件）すべてで一致する。
...
係数（0.01 kg 単位など）を 1 桁でも間違えるとこの関係は崩れるため、答え合わせなしで確定できる。
実装では受信ごとにこの検算を行い、結果をログへ出す（`Sources/BC768Protocol/Measurement.swift`）。

基礎代謝・体内年齢・内臓脂肪レベル・体水分率は値の桁が妥当なだけで、検算できないため推定のまま。
Health Planet の表示値と突き合わせれば確定する。
